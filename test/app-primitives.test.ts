...
describe("swapping the backend", () => {
  it("persists through a custom backend without any component change", () => {
    // The shape a hosted-service adapter would take: read returns the stored
    // JSON, write sends it. Nothing else in the app knows the difference.
    const sent: string[] = [];
    ...
    const collection = createCollection<Item>("k", { revive: reviveRecord, backend: remote });
    collection.replace([{ id: "1", label: "one" }]);

    expect(sent).toHaveLength(1);
    expect(collection.list()).toEqual([{ id: "1", label: "one" }]);

    // A fresh collection over the same backend sees what was written.
    const reopened = createCollection<Item>("k", { revive: reviveRecord, backend: remote });
    expect(reopened.list()).toEqual([{ id: "1", label: "one" }]);
  });

  it("reports an unreachable backend instead of throwing", () => {
    const failures: StorageFailure[] = [];
    const offline = {
      read: () => {
    ...
    // Reads and writes both failed, yet the session stays usable.
    expect(collection.list()).toEqual([{ id: "1", label: "one" }]);
    expect(failures).toEqual(["unavailable", "write-failed"]);
  });
});
