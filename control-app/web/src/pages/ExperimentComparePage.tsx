...
import { ExperimentCompareActionFlow } from "../components/ExperimentCompareActionFlow";
...
function parsePreset(value: string | null): ExperimentComparePreset {
...
export function ExperimentComparePage() {
  ...
  const config = EXPERIMENT_COMPARE_PRESETS[preset];
  ...
  const labels = useMemo(() => experimentCompareLabelMap(preset), [preset]);
  ...
  const matched = compareRuns.filter((c) => c.run).map((c) => c.run!);
      ...
      <section className="panel">
        ...
        {!loading && !error && (
          <>
            ...
            <ExperimentCompareActionFlow runs={matched} labels={labels} />
