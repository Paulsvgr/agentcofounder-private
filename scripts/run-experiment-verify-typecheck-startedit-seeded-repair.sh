#!/usr/bin/env bash
# Seeded 257k startEdit repair: VERIFY only vs VERIFY+TYPECHECK (tsc facts).
...
export HARNESS_SEEDED_FIXTURE_DIR="fixtures/verify-typecheck-startedit-seeded"
IDEA_FILE="$REPO_ROOT/fixtures/verify-typecheck-startedit-seeded/repair-idea.txt"

LOG_DIR="$REPO_ROOT/artifacts/experiments/verify-typecheck-startedit-seeded"
mkdir -p "$LOG_DIR"
...
run_arm() {
  ...
  export RUN_INTERVENTION="verify-typecheck-on-fail-v1"
  for rep in $(seq 1 "$REPS"); do
    export RUN_REP="$rep"
    free_app_port
    echo "=== seeded $arm_name TYPECHECK_ON_FAIL=$typecheck rep $rep / $REPS $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
{
  echo "=== verify-typecheck-startedit seeded repair start $(date -u +%Y-%m-%dT%H:%M:%SZ) arm=$ARM reps=$REPS ==="
  echo "Fixture: $HARNESS_SEEDED_FIXTURE_DIR | treatment adds TYPECHECK TS facts only"
  if [ "$ARM" = "control" ] || [ "$ARM" = "both" ]; then
    run_arm 0 "verify-typecheck-startedit-seeded-control" "control"
  fi
  if [ "$ARM" = "treatment" ] || [ "$ARM" = "both" ]; then
    run_arm 1 "verify-typecheck-startedit-seeded-treatment" "treatment"
  fi
  echo "=== verify-typecheck-startedit seeded repair complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
} 2>&1 | tee -a "$PAIR_LOG"
