#!/usr/bin/env bash
# Experiment SS2 — Scope & sequence v2 — same message, earlier delivery anchor (A/B vs SS1).
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"
LOG_DIR="$REPO_ROOT/artifacts/experiments/ss2-scope-sequence-v2"
mkdir -p "$LOG_DIR"
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-ss2-scope-sequence-v2}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-scope-sequence-v2}"
...
export HARNESS_SCOPE_SEQUENCE_V2="${HARNESS_SCOPE_SEQUENCE_V2:-1}"
...
echo "Experiment SS2 (ss2-scope-sequence-v2) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment SS2 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment SS2 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
