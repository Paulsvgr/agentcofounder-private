#!/usr/bin/env bash
# CSS A/B Arm A — persistence only + root-error-first (CSS vocabulary OFF).
# Product-quality experiment; see docs/v2/control-floor/experiment-css-ab-persist-v1-preregistration.md
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-css-ab-persist-v1-a}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-css-ab-persist-v1-a}"
...
echo "css-ab-persist-v1-a — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "Arm A: PERSISTENCE=1 CSS=0 ROOT_ERROR_FIRST=1 (Error Memory OFF)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "css-ab-persist-v1-a complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "css-ab-persist-v1-a complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
