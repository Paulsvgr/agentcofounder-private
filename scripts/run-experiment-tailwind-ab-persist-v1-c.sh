#!/usr/bin/env bash
# Tailwind A/B Arm C — persistence + preinstalled Tailwind (CSS vocabulary OFF).
# See docs/v2/control-floor/experiment-tailwind-ab-persist-v1-preregistration.md
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-tailwind-ab-persist-v1-c}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-tailwind-ab-persist-v1-c}"
...
echo "tailwind-ab-persist-v1-c — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "Arm C: PERSISTENCE=1 TAILWIND=1 CSS=0 ROOT_ERROR_FIRST=1 (Error Memory OFF)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "tailwind-ab-persist-v1-c complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "tailwind-ab-persist-v1-c complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
