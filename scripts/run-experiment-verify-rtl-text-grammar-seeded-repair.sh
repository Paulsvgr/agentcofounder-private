#!/usr/bin/env bash
# Deterministic seeded Pi repair: same grammar-miss fixture, TEXT_EVIDENCE=0 vs =1.
# Prereq: npm run prove:verify-rtl-text-grammar-seeded-messages
# See docs/v2/control-floor/experiment-verify-rtl-text-evidence-v1-preregistration.md
set -euo pipefail
...
ARM="${1:-both}" # control | treatment | both
REPS="${2:-1}"
...
export HARNESS_SEEDED_FIXTURE_DIR="fixtures/verify-rtl-text-grammar-seeded"
IDEA_FILE="$REPO_ROOT/fixtures/verify-rtl-text-grammar-seeded/repair-idea.txt"

LOG_DIR="$REPO_ROOT/artifacts/experiments/verify-rtl-text-grammar-seeded"
mkdir -p "$LOG_DIR"
...
run_arm() {
  ...
  local exp_id="$2"
  local arm_name="$3"
  export HARNESS_VERIFY_RTL_TEXT_EVIDENCE_V1="$text_ev"
  export RUN_EXPERIMENT="$exp_id"
  export RUN_ARM="$arm_name"
  export RUN_INTERVENTION="verify-rtl-text-evidence-v1-grammar"
  for rep in $(seq 1 "$REPS"); do
    export RUN_REP="$rep"
    free_app_port
    echo "=== seeded $arm_name TEXT_EVIDENCE=$text_ev rep $rep / $REPS $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
    npm run challenge -- --idea-file "$IDEA_FILE" || {
      echo "WARNING: seeded $arm_name rep $rep exited non-zero (continuing)"
    }
    free_app_port
  done
}
...
{
  echo "=== verify-rtl-text grammar seeded repair start $(date -u +%Y-%m-%dT%H:%M:%SZ) arm=$ARM reps=$REPS ==="
  echo "Fixture: $HARNESS_SEEDED_FIXTURE_DIR | TYPECHECK=1 MULTIPLE=1 | TEXT_EVIDENCE arm toggle | no Error Memory"
  if [ "$ARM" = "control" ] || [ "$ARM" = "both" ]; then
    run_arm 0 "verify-rtl-text-grammar-seeded-control" "control"
  fi
  if [ "$ARM" = "treatment" ] || [ "$ARM" = "both" ]; then
    run_arm 1 "verify-rtl-text-grammar-seeded-treatment" "treatment"
  fi
  echo "=== verify-rtl-text grammar seeded repair complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
} 2>&1 | tee -a "$PAIR_LOG"
