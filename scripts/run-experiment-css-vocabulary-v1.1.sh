#!/usr/bin/env bash
# Experiment CSS Vocabulary v1.1 — v2.2 + hardened CSS design system (only intervention).
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-css-vocabulary-v1.1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-css-vocabulary-v1.1}"
export HARNESS_OWNED_VERIFY="${HARNESS_OWNED_VERIFY:-1}"

REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/css-vocabulary-v1.1"
mkdir -p "$LOG_DIR"
...
echo "Experiment CSS Vocabulary v1.1 — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment CSS Vocabulary v1.1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment CSS Vocabulary v1.1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
