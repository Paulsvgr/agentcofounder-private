#!/usr/bin/env bash
# Experiment SS1 — Scope & sequence v1 — v2.2 OFF/OFF + pre-VERIFY scope/sequence nudge only.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"
LOG_DIR="$REPO_ROOT/artifacts/experiments/ss1-scope-sequence-v1"
mkdir -p "$LOG_DIR"
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-ss1-scope-sequence-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-scope-sequence-v1}"
...
export HARNESS_SCOPE_SEQUENCE_V1="${HARNESS_SCOPE_SEQUENCE_V1:-1}"
...
echo "Experiment SS1 (ss1-scope-sequence-v1) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment SS1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment SS1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
