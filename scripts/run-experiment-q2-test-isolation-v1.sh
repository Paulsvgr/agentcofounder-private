#!/usr/bin/env bash
# Experiment Q2 — Test Isolation v1 — v2.2 + test-isolation overlay only (only intervention).
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-q2-test-isolation-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-q2-test-isolation-v1}"
...
export TEMPLATE_TEST_ISOLATION="${TEMPLATE_TEST_ISOLATION:-1}"

REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/q2-test-isolation-v1"
mkdir -p "$LOG_DIR"
...
echo "Experiment Q2 (q2-test-isolation-v1) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment Q2 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment Q2 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
