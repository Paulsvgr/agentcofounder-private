#!/usr/bin/env bash
# Experiment Q2-D — Early VERIFY v1 — v2.2 OFF/OFF + automatic first test mutation VERIFY only.
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-q2-early-verify-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-early-verify-v1}"
...
export HARNESS_EARLY_VERIFY_V1="${HARNESS_EARLY_VERIFY_V1:-1}"
...
REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/q2-early-verify-v1"
mkdir -p "$LOG_DIR"
...
echo "Experiment Q2-D (q2-early-verify-v1) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment Q2-D complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment Q2-D complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
