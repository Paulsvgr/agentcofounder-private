#!/usr/bin/env bash
# verify-rtl-evidence-v1 CONTROL — legacy line-count compact reporter.
# Same default stack as treatment; only HARNESS_VERIFY_RTL_EVIDENCE_V1=0 differs.
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-verify-rtl-evidence-v1-control}"
export RUN_ARM="${RUN_ARM:-control}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-verify-rtl-evidence-v1-legacy}"
...
export HARNESS_VERIFY_RTL_EVIDENCE_V1=0
...
echo "verify-rtl-evidence-v1-control — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "Control: PERSISTENCE=1 TAILWIND=1 ROOT_ERROR_FIRST=1 RTL_EVIDENCE=0 (legacy truncate)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "verify-rtl-evidence-v1-control complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "verify-rtl-evidence-v1-control complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
