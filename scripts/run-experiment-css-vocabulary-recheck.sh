#!/usr/bin/env bash
# CSS vocabulary recheck — v2.2 + TEMPLATE_CSS_VOCABULARY=1 only.
# Purpose: compare cost/cache vs today's pure v22-recheck (~61k median).
set -euo pipefail
...
export TEMPLATE_PERSISTENCE=0
export TEMPLATE_TEST_ISOLATION=0

REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/css-vocabulary-recheck"
mkdir -p "$LOG_DIR"
...
echo "css-vocabulary-recheck — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "css-vocabulary-recheck complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "css-vocabulary-recheck complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
