#!/usr/bin/env bash
# v2.2 recheck — pure control floor (all experiment flags OFF).
# Purpose: verify whether median ~60k / high cache still holds after Sep 2 regressions.
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-v22-recheck}"
export RUN_ARM="${RUN_ARM:-control}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-none}"

# v2.2 baseline only
export HARNESS_OWNED_VERIFY="${HARNESS_OWNED_VERIFY:-1}"

# Explicitly OFF — all later experiments / overlays
...
REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/v22-recheck"
mkdir -p "$LOG_DIR"
...
echo "v22-recheck — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "v22-recheck complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "v22-recheck complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
