#!/usr/bin/env bash
# Root-error-first VERIFY v1 — CSS + persistence + FAIL reordering only.
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-root-error-first-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-root-error-first-v1}"

# CSS + persistence + root-error-first VERIFY formatting
...
# Explicitly OFF — including Error Memory and verify-repair
...
echo "root-error-first-v1 — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "overlays: CSS=1 PERSISTENCE=1 ROOT_ERROR_FIRST_V1=1 (Error Memory OFF, verify-repair OFF)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "root-error-first-v1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "root-error-first-v1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
