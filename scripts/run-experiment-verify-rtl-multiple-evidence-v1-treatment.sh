#!/usr/bin/env bash
# verify-rtl-multiple-evidence-v1 TREATMENT — structured MATCHES PRESENT for multiples.
# Same default stack as control; only HARNESS_VERIFY_RTL_MULTIPLE_EVIDENCE_V1=1 differs.
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-verify-rtl-multiple-evidence-v1-treatment}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-verify-rtl-multiple-evidence-v1}"
...
echo "verify-rtl-multiple-evidence-v1-treatment — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "Treatment: PERSISTENCE=1 TAILWIND=1 ROOT_ERROR_FIRST=1 RTL_EVIDENCE=1 MULTIPLE_EVIDENCE=1 (MATCHES PRESENT)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "verify-rtl-multiple-evidence-v1-treatment complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "verify-rtl-multiple-evidence-v1-treatment complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
