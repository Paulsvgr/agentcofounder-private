#!/usr/bin/env bash
# Experiment Q2-C — Test Authoring Guard v1 — v2.2 OFF/OFF + pre-VERIFY RTL guard only.
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-test-authoring-guard-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-test-authoring-guard-v1}"
export HARNESS_OWNED_VERIFY="${HARNESS_OWNED_VERIFY:-1}"
export HARNESS_VERIFY_REPAIR_V1="${HARNESS_VERIFY_REPAIR_V1:-0}"
export HARNESS_TEST_AUTHORING_GUARD_V1="${HARNESS_TEST_AUTHORING_GUARD_V1:-1}"
...
REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/test-authoring-guard-v1"
mkdir -p "$LOG_DIR"
...
echo "Experiment Q2-C (test-authoring-guard-v1) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment Q2-C complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment Q2-C complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
