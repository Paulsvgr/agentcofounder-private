#!/usr/bin/env bash
# Experiment Tail Sweep v1 — v2.2 + harness-owned final sweep after report.partial.json
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-tail-sweep-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-tail-sweep-v1}"
export HARNESS_OWNED_VERIFY="${HARNESS_OWNED_VERIFY:-1}"
export HARNESS_TAIL_SWEEP_V1="${HARNESS_TAIL_SWEEP_V1:-1}"
...
export HARNESS_CONVERGENCE_INTERVENTION_V1="${HARNESS_CONVERGENCE_INTERVENTION_V1:-0}"
export HARNESS_SCOPE_SEQUENCE_V1="${HARNESS_SCOPE_SEQUENCE_V1:-0}"
export HARNESS_SCOPE_SEQUENCE_V2="${HARNESS_SCOPE_SEQUENCE_V2:-0}"
export HARNESS_SCOPE_SEQUENCE_V2B="${HARNESS_SCOPE_SEQUENCE_V2B:-0}"
...
REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/tail-sweep-v1"
mkdir -p "$LOG_DIR"
...
echo "Experiment Tail Sweep v1 — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment Tail Sweep v1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment Tail Sweep v1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
