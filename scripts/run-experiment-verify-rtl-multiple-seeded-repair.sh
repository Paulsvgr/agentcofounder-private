#!/usr/bin/env bash
# Deterministic seeded Pi repair: same failing fixture, MULTIPLE_EVIDENCE=0 vs =1.
# Prereq: message proof green (npm run prove:verify-rtl-multiple-seeded-messages).
# See docs/v2/control-floor/experiment-verify-rtl-multiple-evidence-v1-preregistration.md
set -euo pipefail
...
export HARNESS_SEEDED_FIXTURE_DIR="fixtures/verify-rtl-multiple-seeded"
IDEA_FILE="$REPO_ROOT/fixtures/verify-rtl-multiple-seeded/repair-idea.txt"

LOG_DIR="$REPO_ROOT/artifacts/experiments/verify-rtl-multiple-seeded"
mkdir -p "$LOG_DIR"
...
run_arm() {
  ...
  local exp_id="$2"
  local arm_name="$3"
  export HARNESS_VERIFY_RTL_MULTIPLE_EVIDENCE_V1="$multiple"
  export RUN_EXPERIMENT="$exp_id"
  export RUN_ARM="$arm_name"
  export RUN_INTERVENTION="verify-rtl-multiple-seeded"
  for rep in $(seq 1 "$REPS"); do
    export RUN_REP="$rep"
    free_app_port
    echo "=== seeded $arm_name MULTIPLE=$multiple rep $rep / $REPS $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
{
  echo "=== verify-rtl-multiple seeded repair start $(date -u +%Y-%m-%dT%H:%M:%SZ) arm=$ARM reps=$REPS ==="
  echo "Fixture: $HARNESS_SEEDED_FIXTURE_DIR | RTL_EVIDENCE=1 | no Error Memory | no prompt edits"
  if [ "$ARM" = "control" ] || [ "$ARM" = "both" ]; then
    run_arm 0 "verify-rtl-multiple-seeded-control" "control"
  fi
  if [ "$ARM" = "treatment" ] || [ "$ARM" = "both" ]; then
    run_arm 1 "verify-rtl-multiple-seeded-treatment" "treatment"
  fi
  echo "=== verify-rtl-multiple seeded repair complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
} 2>&1 | tee -a "$PAIR_LOG"
