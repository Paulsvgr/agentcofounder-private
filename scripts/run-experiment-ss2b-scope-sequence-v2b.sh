#!/usr/bin/env bash
# Experiment SS2b — Scope & sequence v2b — same message, broad pre-code anchor (A/B vs SS1/SS2).
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REPO_ROOT"
LOG_DIR="$REPO_ROOT/artifacts/experiments/ss2b-scope-sequence-v2b"
mkdir -p "$LOG_DIR"
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-ss2b-scope-sequence-v2b}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-scope-sequence-v2b}"
...
export HARNESS_SCOPE_SEQUENCE_V2B="${HARNESS_SCOPE_SEQUENCE_V2B:-1}"
...
echo "Experiment SS2b (ss2b-scope-sequence-v2b) — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment SS2b complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment SS2b complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
