#!/usr/bin/env bash
# Product-quality build-contract v1: control (flag=0) vs treatment (flag=1).
# Same idea file, locked KEEP stack, hard-stop OFF.
# See docs/v2/control-floor/experiment-product-quality-contract-v1-preregistration.md
set -euo pipefail
...
ARM="${1:-both}"
REPS="${2:-1}"
IDEA_FILE="${3:-$REPO_ROOT/contract-public/development-idea.txt}"
...
LOG_DIR="$REPO_ROOT/artifacts/experiments/product-quality-contract-v1"
mkdir -p "$LOG_DIR"
...
run_arm() {
  ...
  local exp_id="$2"
  local arm_name="$3"
  export HARNESS_PRODUCT_QUALITY_CONTRACT_V1="$flag"
  export RUN_EXPERIMENT="$exp_id"
  export RUN_ARM="$arm_name"
  export RUN_INTERVENTION="product-quality-contract-v1"
  for rep in $(seq 1 "$REPS"); do
    export RUN_REP="$rep"
    free_app_port
    echo "=== $arm_name QUALITY_CONTRACT=$flag rep $rep / $REPS $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
    npm run challenge -- --idea-file "$IDEA_FILE" || {
      echo "WARNING: $arm_name rep $rep exited non-zero (continuing)"
    }
    free_app_port
  done
}
...
{
  echo "=== product-quality-contract-v1 start $(date -u +%Y-%m-%dT%H:%M:%SZ) arm=$ARM reps=$REPS idea=$IDEA_FILE ==="
  echo "Locked KEEP stack ON; hard-stop OFF; no seeded fixture; judge-score track"
  if [ "$ARM" = "control" ] || [ "$ARM" = "both" ]; then
    run_arm 0 "product-quality-contract-v1-control" "control"
  fi
  if [ "$ARM" = "treatment" ] || [ "$ARM" = "both" ]; then
    run_arm 1 "product-quality-contract-v1-treatment" "treatment"
  fi
  echo "=== product-quality-contract-v1 complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
} 2>&1 | tee -a "$PAIR_LOG"
