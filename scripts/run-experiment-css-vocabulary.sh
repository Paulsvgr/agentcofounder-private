#!/usr/bin/env bash
# Experiment CSS Vocabulary v1 — v2.2 + preinstalled CSS design system (only intervention).
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-css-vocabulary-v1}"
...
echo "Experiment CSS Vocabulary v1 — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment CSS Vocabulary v1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment CSS Vocabulary v1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
