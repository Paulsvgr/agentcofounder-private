#!/usr/bin/env bash
# CSS vocabulary v1.1 + persistence-v1 — only those two overlays.
# Purpose: keep ~50k CSS cost while fixing systematic refresh/persist failures.
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-css-persistence-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-css-persistence-v1}"

# v2.2 + CSS vocabulary + persistence primitive
export HARNESS_OWNED_VERIFY="${HARNESS_OWNED_VERIFY:-1}"
export TEMPLATE_CSS_VOCABULARY=1
export TEMPLATE_PERSISTENCE=1

# Explicitly OFF — all other experiments / overlays
...
REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/css-persistence-v1"
mkdir -p "$LOG_DIR"
...
echo "css-persistence-v1 — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
echo "overlays: TEMPLATE_CSS_VOCABULARY=1 TEMPLATE_PERSISTENCE=1 (all other experiment flags OFF)" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "css-persistence-v1 complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "css-persistence-v1 complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
