#!/usr/bin/env bash
# Experiment B — resource-slice-data-v1 (local-storage-collection treatment arm).
set -euo pipefail
...
export RUN_EXPERIMENT="${RUN_EXPERIMENT:-resource-slice-data-v1}"
export RUN_ARM="${RUN_ARM:-treatment}"
export RUN_INTERVENTION="${RUN_INTERVENTION:-resource-slice-data}"

REPS="${1:-5}"
LOG_DIR="$REPO_ROOT/artifacts/experiments/resource-slice-data-v1"
mkdir -p "$LOG_DIR"
...
echo "Experiment B — experiment=$RUN_EXPERIMENT arm=$RUN_ARM intervention=$RUN_INTERVENTION reps=$REPS" | tee -a "$LOG_FILE"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "Experiment B complete ($REPS/$REPS OK). Log: $LOG_FILE" | tee -a "$LOG_FILE"
else
  OK_COUNT=$((REPS - FAILURES))
  echo "Experiment B complete ($OK_COUNT/$REPS OK, $FAILURES failed). Log: $LOG_FILE" | tee -a "$LOG_FILE"
  exit 1
fi
