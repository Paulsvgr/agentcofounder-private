#!/usr/bin/env bash
# Hard-stop vs real 257k-class post-green bait (unused import + default title).
...
export HARNESS_SEEDED_FIXTURE_DIR="fixtures/hard-stop-257k-postgreen"
IDEA_FILE="$REPO_ROOT/fixtures/hard-stop-257k-postgreen/repair-idea.txt"

LOG_DIR="$REPO_ROOT/artifacts/experiments/hard-stop-257k-postgreen"
mkdir -p "$LOG_DIR"
...
run_arm() {
  ...
  export RUN_INTERVENTION="hard-stop-after-green-v1-257k"
  for rep in $(seq 1 "$REPS"); do
    export RUN_REP="$rep"
    free_app_port
    echo "=== hard-stop-257k $arm_name HARD_STOP=$hardstop rep $rep / $REPS $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
    npm run challenge -- --idea-file "$IDEA_FILE" || {
      echo "WARNING: hard-stop-257k $arm_name rep $rep exited non-zero (continuing)"
    }
    free_app_port
  done
}
...
{
  echo "=== hard-stop-257k-postgreen start $(date -u +%Y-%m-%dT%H:%M:%SZ) arm=$ARM reps=$REPS ==="
  echo "Fixture: green @46 bait (unused fireEvent + default title). Neutral idea. Only HARD_STOP differs."
  if [ "$ARM" = "control" ] || [ "$ARM" = "both" ]; then
    run_arm 0 "hard-stop-257k-postgreen-control" "control"
  fi
  if [ "$ARM" = "treatment" ] || [ "$ARM" = "both" ]; then
    run_arm 1 "hard-stop-257k-postgreen-treatment" "treatment"
  fi
  echo "=== hard-stop-257k-postgreen complete $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
} 2>&1 | tee -a "$PAIR_LOG"
